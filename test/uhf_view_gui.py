...
class uhf_view_gui(gr.top_block, Qt.QWidget):

    def __init__(self, freq=435.85e6, gain=30, gs_name='GS1', sat_name='NOAA15', sig_name='SIGNAL'):
        gr.top_block.__init__(self, "Uhf View Gui")
        Qt.QWidget.__init__(self)
        self.setWindowTitle("Uhf View Gui")
        ...
        self.settings = Qt.QSettings("GNU Radio", "uhf_view_gui")
...
def main(top_block_cls=uhf_view_gui, options=None):
    if options is None:
    ...
    tb = top_block_cls(freq=options.freq, gain=options.gain, gs_name=options.gs_name, sat_name=options.sat_name, sig_name=options.sig_name)
    tb.start()
    tb.show()
